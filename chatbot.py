...
def generate_response(initial_context):
    ...
    end_token = app.config["end_token"]
    tokens = pr.generate(model, initial_context, end_token)
    print("TOKENS: ", model.detokenize(tokens))
    generated_tokens = tokens[len(initial_context)-1:]
    #print(list(tokens))
    ...
    generated_text = generated_text.strip(model.detokenize([end_token]))
...
if __name__ == "__main__":
    ...
    end_token = model.tokenizer("©")[0]
    app.config["end_token"] = end_token
